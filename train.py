...
from model import GAN, get_encoder_weights, load_trained_model, pretrain_discriminator
...
def build_generator_kwargs():
    return {
        "use_swin_deep": cfg.get("USE_SWIN_DEEP", False),
        "swin_window_size": cfg.get("SWIN_WINDOW_SIZE", 8),
        "swin_enc3_depth": cfg.get("SWIN_ENC3_DEPTH", 2),
        "swin_enc4_depth": cfg.get("SWIN_ENC4_DEPTH", 2),
        "swin_num_heads_enc3": cfg.get("SWIN_NUM_HEADS_ENC3", 8),
        "swin_num_heads_enc4": cfg.get("SWIN_NUM_HEADS_ENC4", 8),
        "swin_attn_dim_enc3": cfg.get("SWIN_ATTN_DIM_ENC3", 256),
        "swin_attn_dim_enc4": cfg.get("SWIN_ATTN_DIM_ENC4", 256),
        "swin_mlp_ratio": cfg.get("SWIN_MLP_RATIO", 4.0),
        "swin_dropout": cfg.get("SWIN_DROPOUT", 0.0),
        "swin_attn_dropout": cfg.get("SWIN_ATTN_DROPOUT", 0.0),
    }
...
def save_checkpoint_as_artifact(epoch, model, run_id, artifact_base_name="checkpoint"):
    ...
    torch.save({
        ...
        'run_id': run_id,
        'generator_kwargs': model.net_G.arch_kwargs,
    }, checkpoint_file)
...
def evaluate_seg_on_val(GAN_model, val_dl):
    if not GAN_model.use_segmentation:
        return 0.0
    ...
    with torch.no_grad():
        for data in val_dl:
            GAN_model.setup_input(data)
            GAN_model.forward()
            total_seg += GAN_model._compute_seg_loss().item()
            ...
            for warmup_epoch in range(warmup_epochs):
                ...
                for data in tqdm(train_dl, desc=f"Warmup Epoch {warmup_epoch + 1}"):
                    ...
                    if step_warmup % log_interval == 0:
                        with torch.no_grad():
                            bs = min(cfg["BATCH_SIZE"], len(data['L']))
                            ...
                            GAN_model.forward()
                            fake_imgs = log_image_wandb(GAN_model.L, GAN_model.fake_color, num=bs, captions=caps_train)
                            real_imgs = log_image_wandb(GAN_model.L, GAN_model.ab, num=bs, captions=caps_train)
                        wandb.log({
...
def train_from_scratch():
    ...
    net_GAN = GAN(
        ...
        generator_kwargs=build_generator_kwargs(),
    )
